...
论证构造先区分原始研究需求、作者选定的目标与条件、有证据支持的技术障碍，以及所选方案引入的后续问题。缺少普遍缺口证据时，可以清楚提出特定设计目标；贡献需说明新设计、研究对象或实证认识，常规推导、重拟合和检查通过不自动成为创新。
...
语义验收沿用生产与核验分离：核对者从实际文本恢复研究目标、重要性、困难、设计回应、章节关系及结论依据。主张先行是默认表达方向，各类段落可以有不同节奏。实际行为测试使用 [通用样本](evals/argument-fixtures.md)，与格式、链接等结构检查分开判定。
