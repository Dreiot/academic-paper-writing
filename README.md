...
必要全文无法取得时，Skill 会依次尝试出版社、DOI、开放获取版本、作者稿或预印本、机构知识库、本地 Zotero 和已授权连接。首次人工核对完成后，默认自动完成查询、会话复用及浏览器管理的常规重新登录；会话超时、登录页和已核验路径内的正常 SSO 跳转不单独触发询问。只有外部验证码、MFA、CAPTCHA、必须手动输入的信息或新的授权决定等确实需要人工介入的步骤，才短暂中断，请用户在浏览器中完成；之后接续原查询，不再重复询问。缺少订阅权限时先尝试合法替代来源。不会要求用户在对话中发送密码、Cookie、访问令牌或验证码。
