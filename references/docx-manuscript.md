...
- Keep the manuscript black and white and use the smallest practical style set. Distinguish hierarchy through restrained size, weight, spacing, and numbering rather than decorative rules, colored headings, shaded cards, floating panels, or report-style visual branding.
...
- Follow the journal or established manuscript treatment for tables. Otherwise use a restrained academic three-line treatment without vertical rules for Chinese first drafts; use a full grid only when the data structure, accessibility needs, or target venue calls for it. Do not apply `Table Grid` merely because it is a familiar built-in name.
- Size columns deliberately, repeat multi-page header rows, vertically center cells, left-align narrative labels, and center or decimal-align compact numeric values as appropriate.
...
After every layout-sensitive correction, render the latest file again and inspect every page at readable zoom. If no visual renderer is available, complete structural OOXML checks, state that visual QA was not completed, and do not claim that the document passed rendering.
