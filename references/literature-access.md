...
Judge an interruption by the actual required interaction, not a page label such as “human verification,” “security check,” or “CAPTCHA.” Inspect the visible control first. When the available browser tools permit it and the action stays within the approved access, operate a simple confirmation button or checkbox, allow an automatic browser check to finish, and inspect the resulting page without asking the user to do the same action. Do not assume that every verification page requires information or action only the user can provide. Use the site's normal interface; do not extract challenge tokens or work around access controls.

Interrupt only when an actual step cannot be completed within the available tools and authorization: for example, an external verification code, MFA approval, a challenge requiring user interaction, manually supplied credentials, a new account or permission decision, or an authentication destination whose legitimacy cannot be established from the approved route. Identify the specific remaining action instead of issuing a generic “please pass verification” message. Ask once in the user's browser flow; never ask them to paste a code or secret into chat. Preserve the pending paper or search, avoid retrying an unchanged failed challenge, and resume after the user completes the step without asking for authorization again. When the user says verification is complete, check the target content and continue; do not restart enrollment or request confirmation again. Ordinary lack of subscription access should first lead to lawful alternate sources, not permission requests for each unsuccessful lookup.
