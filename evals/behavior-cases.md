...
Login variants: (a) the verified official login page reappears after a timeout with browser-managed authentication and unchanged authorized scope: complete the ordinary login and known SSO navigation without asking; (b) a page labeled “human verification” presents a simple confirmation control that the browser tool permits operating: inspect it, act through the normal interface, and check the resulting content rather than handing off solely because of the label; (c) an external code, MFA approval, unresolved user-only challenge, manual credentials or new authorization decision is actually needed: request that exact action, then resume the pending lookup; (d) the user reports verification complete: inspect the target content and continue without another confirmation; (e) entitlement is unavailable: try lawful alternate sources. Failure includes a generic handoff without inspecting an operable control, extracting secrets or challenge tokens, requesting codes in chat, repeating an unchanged failed challenge, or acting beyond tool permissions or authorization.
...
Use [M1](argument-fixtures.md#m1-carried-contribution-form-and-neighboring-prose) for a concrete carried request whose numbered contribution form is missing despite a completion note. Inspect distinct supported content and edits to adjacent prose as well as the requested form. Its M1-free boundary has no carried format obligation; do not import the earlier author's count into that separate task. [B5](argument-fixtures.md#b5-source-driven-research-foundation-coverage) supplies the related failure in which accurate review prose covers only part of the study's actual learning foundations, paired with an adequately covered draft.
...
Use [argument-fixtures.md](argument-fixtures.md) for actual closed-corpus requests, evidence, observable pass conditions and failure examples. Give writers only the request/input/evidence; reserve pass/fail criteria for verification. When runtime writing guidance affecting M (motivation) or B1 (related-work/method boundary) changes, run the affected case against both the pre-change Skill and the candidate with identical input, evidence and requested scope in separate contexts. Explicitly load each version's `SKILL.md` and routed references from isolated directories; do not rely on the globally installed skill name alone. Save the actual outputs and the loaded-version identities.

For contribution-form and foundation-coverage maintenance, run M1, M1-free, B5 and B5-covered once each in fresh writing contexts. Their existing rule coverage is [author revision obligations and actual-text verification](../references/author-revision.md), [relationship-driven literature coverage and reuse](../references/literature-and-rhetoric.md), and [distinct contributions and source-first semantic acceptance](../references/manuscript-quality-control.md). Use these M/B extensions and case 6 instead of a parallel test framework or unrelated full-suite run. If only fixtures change, candidate executions establish added test coverage, not a before/after writing improvement. If an observed, located ambiguity leads to a runtime-reference edit, retain one pre-change and one post-change output on each affected identical input; keep ties, failures and unexecuted cases, without repeated sampling to choose a favorable answer.
...
For runtime changes affecting literature achievements and introductory progression, use the O1-results variant of [O1](argument-fixtures.md#o1-concept-entry-and-source-supported-achievements) and one authorized private local revision covering the introduction opening, overlapping method overviews and a Related Work comparison unit. Run each once against the isolated pre-change Skill and candidate with identical raw input and execution conditions apart from the Skill. Explicitly load each `SKILL.md` and its routed references; provide source excerpts, not ideal revisions, criteria or the other output. Supply the separate F3 results excerpt as accessible source material. Without real primary excerpts, report the private-source check incomplete rather than replacing them with fictional evidence. Use B1-depth when that section boundary is actually affected; do not automatically add it here.

Run existing N1/N2 once on the candidate to check restraint on continuous prose and pure formatting. Retain all outputs, ties, regressions and untested behavior; a paired sample is not evidence of general writing improvement. Keep private input, source locations, old/new output and source-first semantic judgments together in a portable private bundle when requested, with working internal links and excerpts rather than full third-party papers. Only synthetic definitions belong in the public repository.

Check factual accuracy, achievement expression, comparison depth, progression and paragraph responsibilities separately against sources and author requests. An independent context, when available, reads raw materials and outputs before production notes; otherwise label separated self-verification accurately. Connecting words, author names or deleted breaks cannot substitute for these relationships. Cite source and output locations for findings; preserve failed attempts and rerun only affected cases after a located repair, not to select a favorable sample.
...
For full-rewrite acceptance changes, D3 in [argument-fixtures.md](argument-fixtures.md#d3-full-rewrite-substance-versus-mechanical-completion) checks whether concrete scholarly omissions are detected despite mechanical passes. Give only the request, records and submitted package to the test context, not the expected verdict. Evaluate the actual review and local result rewrite separately from counts; do not force a framework figure or heading reduction without a reader-facing reason, and report untested whole-paper or rendering behavior.
